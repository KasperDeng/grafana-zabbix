...
class EventPanelEditorCtrl {
  ...
  constructor($scope, $rootScope, uiSegmentSrv, datasourceSrv, templateSrv, popoverSrv) {
    ...
    this.sortByFields = [
      { text: 'age-asc',  value: 'age-asc' },
      { text: 'age-dsc',  value: 'age-dsc' },
  ...
  suggestHosts() {
    let groupFilter = this.datasource.replaceTemplateVars(this.panel.events.group.filter);
  ...
  suggestApps() {
    let groupFilter = this.datasource.replaceTemplateVars(this.panel.events.group.filter);
    let hostFilter = this.datasource.replaceTemplateVars(this.panel.events.host.filter);
...
export function eventPanelEditor() {
  return {
    restrict: 'E',
    scope: true,
    templateUrl: 'public/plugins/alexanderzobnin-zabbix-app/panel-events/editor.html',
    controller: EventPanelEditorCtrl,
  };
}
